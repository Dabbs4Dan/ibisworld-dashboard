...
- **localStorage** for persistence — four keys, all in one logical namespace:
  ...
  - Also in `ibis_local` (per-account, v41): `customOpp` (manual My Opp free text), `acctPriority` now holds Action-stage keys (`new_sequence`/`active_opp`/…/`unresponsive`/`won`/`nurture`/`''`), `actionHeadline` now editable from the Accounts tab too (shared with Action tab).
