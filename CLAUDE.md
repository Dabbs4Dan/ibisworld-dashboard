...
- **Turn on GitHub direct backup (Dan's one-time action):** GitHub → Settings → Developer settings → **Fine-grained tokens** → Generate new → Resource owner = Dabbs4Dan, Repository access = **Only select repositories → ibisworld-dashboard**, Permissions → **Contents: Read and write**, expiry your choice → Generate → copy `github_pat_...`. Then in the dashboard: 🛡 shield icon → **Connect GitHub** → paste. The token stays on the machine only and is never in any backup file.
