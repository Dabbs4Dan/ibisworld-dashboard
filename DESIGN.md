...
|---|---|---|
| v44.1 | 2026-07-10 | **Backups panel declutter + guided reconnect.** Default view is now a single clean status card — `✅ Everything is backed up` + two dotted lines (`💻 This browser — saved Xm ago` / `☁️ GitHub cloud — saved Xm ago`, reusing `.bp-layer-dot` inline) + a "two independent copies" caption — then one `.bp-restore-btn`, then the `Show details` toggle. **Everything else moved under details:** the storage bar, snapshot list, and a new **"Where your data is saved"** section (3 `.bp-layer` rows — browser / GitHub / optional local folder, each with an inline `.bp-action-btn`), plus Advanced. Removed the two always-on colored banners (folder + GitHub) from the default view and the dishonest "4 protection layers" list (it double-counted the folder). New `#guardian-reconnect` toast (indigo `#eef2ff`/`#c7d2fe`, bottom-center, matches `#guardian-toast`) with a `🔑` + Reconnect/✕ — shown after a full-restore recovery when the GitHub token was wiped. No new tokens/colors. |
| v44 | 2026-07-10 | **🛡 Data Guardian UI.** `#guardian-overlay` — full-screen restore veil (`rgba(15,23,42,.72)` + blur, centered white 12px card, 🛡 glyph) shown only while auto-recovery is fetching. `#guardian-toast` — bottom-center dark pill (`#0f172a`, 10px radius, high shadow) confirming a recovery, with a white **Undo** button on partial repairs (auto-dismisses after 9s on full restores). `#guardian-shrink-banner` — top amber gradient banner (`#92400e→#b45309`) when the sanity gate freezes a shrunken backup, with "Recover my data" (white) / "This is correct" (ghost) actions; mirrors the red `#save-failure-banner` pattern. **Backups panel — GitHub direct-backup section:** indigo CTA (`#eef2ff`/`#4f46e5` left-border, "Connect GitHub" `#4f46e5` button) when off; green status (`#f0fdf4`/`#16a34a`, "Push now" + "Disconnect") when on — matches the existing FSA CTA / green-active pattern. All reuse `.bp-section` / `.bp-action-btn` tokens. No new colors outside the palette. |
| v43.1 | 2026-07-09 | **Post-session polish.** `content-visibility:auto` + `contain-intrinsic-size:auto 46px` on `.acct-main-table tbody tr` (off-screen rows skip render — fixes the Status-expand paint lag; sticky header/row-num still work). Closed a 1px gap between the two sticky header rows: `.hf-labels th` (+ `.row-num-cell`) `top:42px`→`40px` (bubble strip is 41px; slight overlap, no body-cell peek-through). **Stats bar** (`#stats-accounts`): Total · ✓ Keep · My Opps · Other Opps · Existing Clients · Last Import (dropped Tier 1 + Avg Intent); `.stat-item` spacing 32px→22px; `.csv-upload-stat` no longer `margin-left:auto` (removed the big middle gap). Collapsed Status column 28px→20px, toggle button padding tightened. |
