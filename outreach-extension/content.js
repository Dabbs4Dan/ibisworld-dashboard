...
(function () {
  ...
  function normFolder(text) {
    ...
    // \uFE0E\uFE0F = text/emoji variation selectors. \p{Mn} = nonspacing marks.
    // CRITICAL: ❄️ = U+2744 + U+FE0F. Without stripping U+FE0F, the invisible char
    // survives and breaks exact matching ("️ Winback" !== "Winback"). This was the Winback bug.
    return text
      .replace(/^[\s\p{Extended_Pictographic}\p{So}\p{Mn}\p{Cf}\uFE0E\uFE0F\-–—★→✓•]+/u, '') // strip leading
      .replace(/[\s\p{Extended_Pictographic}\p{So}\p{Mn}\p{Cf}\uFE0E\uFE0F\-–—★→✓•]+$/u, '') // strip trailing
      .trim();
  }
